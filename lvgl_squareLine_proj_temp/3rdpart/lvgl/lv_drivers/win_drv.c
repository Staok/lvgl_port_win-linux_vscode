...
static void do_register(void)
{
    ...
    // lv_disp_draw_buf_init(&disp_draw_buf_1, buf1_1, NULL, WINDOW_HOR_RES * 100);   /*Initialize the display buffer*/ // why mistake here???
    lv_disp_draw_buf_init(&disp_buf_1, buf1_1, NULL, WINDOW_HOR_RES * 100);
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        if(fbp == NULL)
            return 1;
        // SetTimer(hwnd, 0, 10, (TIMERPROC)lv_task_handler); not work here
